...
impl BinanceRestFetcher {
        ...
        if let Some(tickers) = json.as_array() {
            for ticker in tickers {
                ...
                let ask = ticker["askPrice"]
                    ...
                    .and_then(|s| s.parse::<f64>().ok())
                    .unwrap_or(0.0);
                let bid_size = ticker["bidQty"]
                    .as_str()
                    .and_then(|s| s.parse::<f64>().ok())
                    .unwrap_or(0.0);
                let ask_size = ticker["askQty"]
                    .as_str()
                    .and_then(|s| s.parse::<f64>().ok())
                    .unwrap_or(0.0);
    ...
    pub async fn fetch_orderbooks(symbols: &[String]) -> OrderbookResult {
        ...
        debug!(
            "Binance: Fetching {} orderbooks via book ticker API",
            symbols.len()
        );
        ...
        let result: OrderbookResult = all_tickers
            .into_iter()
            ...
            .map(|product_id| {
                ...
                async move {
                    match Self::fetch_orderbook(&product_id).await {
                        ...
                        Err(e) => {
                            debug!(
                                "Coinbase: Failed to fetch orderbook for {}: {}",
                                product_id, e
                            );
                            None
                        }
...
impl BybitRestFetcher {
    ...
    async fn fetch_all_tickers() -> OrderbookResult {
        ...
        if let Some(list) = json["result"]["list"].as_array() {
            for ticker in list {
                ...
                let bid = ticker["bid1Price"]
                    .as_str()
                    .and_then(|s| s.parse::<f64>().ok())
                    .unwrap_or(0.0);
                let ask = ticker["ask1Price"]
                    .as_str()
                    .and_then(|s| s.parse::<f64>().ok())
                    .unwrap_or(0.0);
                let bid_size = ticker["bid1Size"]
                    .as_str()
                    .and_then(|s| s.parse::<f64>().ok())
                    .unwrap_or(0.0);
                let ask_size = ticker["ask1Size"]
                    .as_str()
                    .and_then(|s| s.parse::<f64>().ok())
                    .unwrap_or(0.0);
    ...
    pub async fn fetch_orderbooks(symbols: &[String]) -> OrderbookResult {
        ...
        debug!(
            "Bybit: Fetching {} orderbooks via tickers API",
            symbols.len()
        );
...
impl GateIORestFetcher {
    ...
    async fn fetch_single_ticker(currency_pair: &str) -> Option<(String, OrderbookEntry)> {
        ...
        let bid_size = ticker["highest_size"]
            .as_str()
            .and_then(|s| s.parse::<f64>().ok())
            .unwrap_or(0.0);
        let ask_size = ticker["lowest_size"]
            .as_str()
            .and_then(|s| s.parse::<f64>().ok())
            .unwrap_or(0.0);
    ...
    pub async fn fetch_orderbooks(currency_pairs: &[String]) -> OrderbookResult {
        ...
        debug!(
            "GateIO: Fetching {} orderbooks via tickers API (single call)",
            currency_pairs.len()
        );

        // Build a set of requested pairs for O(1) lookup
        let requested: std::collections::HashSet<String> =
            currency_pairs.iter().map(|p| p.to_uppercase()).collect();
        ...
        for ticker in tickers {
            ...
            let bid = ticker["highest_bid"]
                .as_str()
                .and_then(|s| s.parse::<f64>().ok())
                .unwrap_or(0.0);
            let ask = ticker["lowest_ask"]
                .as_str()
                .and_then(|s| s.parse::<f64>().ok())
                .unwrap_or(0.0);
        ...
        let stablecoin_pairs: Vec<String> = pairs_needing_depth
            .iter()
            .filter(|p| {
                let upper = p.to_uppercase();
                upper.starts_with("USDT_")
                    || upper.starts_with("USDC_")
                    || upper.starts_with("DAI_")
                    || upper.starts_with("TUSD_")
                    || upper.starts_with("BUSD_")
                    || upper.starts_with("FDUSD_")
            })
            .cloned()
            .collect();

        debug!(
            "GateIO: Got {} prices, {} stablecoins need depth fetch (others via WebSocket)",
            result.len(),
            stablecoin_pairs.len()
        );

        // Fetch depth only for stablecoins (small number, no batching needed)
        if !stablecoin_pairs.is_empty() {
            let futures: Vec<_> = stablecoin_pairs
                .iter()
            ...
            debug!(
                "GateIO: Fetched depth for {}/{} stablecoins",
                success_count,
                stablecoin_pairs.len()
            );
        }
...
impl BithumbRestFetcher {
    ...
    pub async fn fetch_orderbook(symbol: &str) -> Result<OrderbookEntry, FeedError> {
        ...
        let best_bid = bids
            .first()
            .ok_or_else(|| FeedError::ParseError("No bids".to_string()))?;
        let bid = best_bid["price"]
            .as_str()
            .ok_or_else(|| FeedError::ParseError("Invalid bid price".to_string()))?
            .parse::<f64>()
            .map_err(|_| FeedError::ParseError("Invalid bid price".to_string()))?;
        let bid_size = best_bid["quantity"]
            .as_str()
            .ok_or_else(|| FeedError::ParseError("Invalid bid size".to_string()))?
            .parse::<f64>()
            .map_err(|_| FeedError::ParseError("Invalid bid size".to_string()))?;
        ...
        let best_ask = asks
            .first()
            .ok_or_else(|| FeedError::ParseError("No asks".to_string()))?;
        let ask = best_ask["price"]
            .as_str()
            .ok_or_else(|| FeedError::ParseError("Invalid ask price".to_string()))?
            .parse::<f64>()
            .map_err(|_| FeedError::ParseError("Invalid ask price".to_string()))?;
        let ask_size = best_ask["quantity"]
            .as_str()
            .ok_or_else(|| FeedError::ParseError("Invalid ask size".to_string()))?
            .parse::<f64>()
            .map_err(|_| FeedError::ParseError("Invalid ask size".to_string()))?;
...
pub async fn fetch_all_initial_orderbooks(
    ...
    debug!(
        "Binance: Fetched {} orderbooks",
        all_orderbooks
            .iter()
            .filter(|((ex, _), _)| ex == "Binance")
            .count()
    );
    ...
    debug!(
        "Coinbase: Fetched {} orderbooks",
        all_orderbooks
            .iter()
            .filter(|((ex, _), _)| ex == "Coinbase")
            .count()
    );
    ...
    debug!(
        "Bybit: Fetched {} orderbooks",
        all_orderbooks
            .iter()
            .filter(|((ex, _), _)| ex == "Bybit")
            .count()
    );
    ...
    debug!(
        "GateIO: Fetched {} orderbooks",
        all_orderbooks
            .iter()
            .filter(|((ex, _), _)| ex == "GateIO")
            .count()
    );

    debug!(
        "Total: Fetched {} orderbooks from all exchanges",
        all_orderbooks.len()
    );
    all_orderbooks
}
