...
impl SymbolMappings {
    ...
    /// Get all excluded (exchange, symbol) pairs.
    pub fn excluded_pairs(&self) -> Vec<(&str, &str)> {
        self.mappings
            .iter()
            .filter(|m| m.exclude)
            .map(|m| (m.exchange.as_str(), m.symbol.as_str()))
            .collect()
    }

    /// Check if a symbol has conflicting canonical names across exchanges.
    /// Returns true if the same symbol has different canonical names on different exchanges,
    /// meaning it represents different coins and should be excluded from cross-exchange arbitrage.
    pub fn has_conflict(&self, symbol: &str) -> bool {
        let canonical_names: Vec<_> = self
            .mappings
            .iter()
        ...
        if canonical_names.len() < 2 {
            return false;
        }

        // Check if all canonical names are the same
        let first = &canonical_names[0];
        canonical_names.iter().any(|name| name != first)
    }
...
mod tests {
    ...
    fn test_excluded_pairs() {
        ...
        mappings.mappings.push(SymbolMapping {
            ...
            symbol: "GTC".to_string(),
            canonical_name: "Game.com".to_string(),
            exclude: true,
            notes: None,
        });
        mappings.mappings.push(SymbolMapping {
        ...
        let excluded = mappings.excluded_pairs();
        assert_eq!(excluded.len(), 1);
        assert_eq!(excluded[0], ("GateIO", "GTC"));
    }
    ...
    fn test_has_conflict() {
        ...
        assert!(mappings.has_conflict("GTC"));
        assert!(!mappings.has_conflict("BTC")); // No mappings
    }
}
